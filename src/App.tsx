...
import { emptyChainState, emptyLocalizationState, emptyReviewState, type ChainState, type LocalizationState, type ReviewState } from "./exercise-state";
...
function CodeWorkspace({ repository, lesson, currentFile, content, line, workspaceMode, fontBoost, trail, chainState, onChainState, localizationState, onLocalizationState, reviewState, onReviewState, onOpen, onSelection, onWorkspaceMode, resolution, resolutionBusy, onResolve }: {
  repository: Repository;
  lesson: Lesson;
  currentFile: RepoFile | null;
  content: string;
  ...
  return (
    <main className="code-workspace panel-border">
      ...
      <section className="editor-column">
        ...
        {workspaceMode === "chains" && <Suspense fallback={<div className="panel-loading">Loading exercises…</div>}><LazyCallChainPanel repository={repository} state={chainState} onState={onChainState} onAnchor={(path, targetLine) => { const file = repository.files.find((item) => item.path === path); if (file) { onWorkspaceMode("code"); onOpen(file, targetLine); } }} /></Suspense>}
